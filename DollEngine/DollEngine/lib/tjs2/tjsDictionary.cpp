...
void tTJSDictionaryNI::Assign(iTJSDispatch2 * dsp, bool clear)
{
	...
	else
	{
        ...
        tTJSVariantClosure value(&countCallback, NULL);
        dsp->EnumMembers(TJS_IGNOREPROP, &value, dsp);
        ...
        tTJSVariantClosure value2(&callback, NULL);
        dsp->EnumMembers(TJS_IGNOREPROP, &value2, dsp);
	...
	iTJSTextWriteStream & stream, const ttstr &indentstr)
{
    ...
    tTJSVariantClosure value(&callback, NULL);
    Owner->EnumMembers(TJS_IGNOREPROP, &value, Owner);
...
void tTJSDictionaryNI::SaveStructuredBinary(std::vector<iTJSDispatch2 *> &stack, tTJSBinaryStream &stream )
{
	...
	callback.Stream = &stream;
    tTJSVariantClosure value2(&callback, NULL);
    Owner->EnumMembers(TJS_IGNOREPROP, &value2, Owner);
}
//---------------------------------------------------------------------------
