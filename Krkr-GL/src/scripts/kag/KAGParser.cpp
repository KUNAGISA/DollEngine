...
,m_tag(null)
,m_isScripts(false)
,m_encoding(ENCODING_UTF8_WITHOUT_BOM)
...
,m_isMakeMacro(false)
{
    setComName(L"KAGParser");
    setTriggerType(COM::TRIGGER_UPDATE);
...
KAGStorage* KAGParser::loadScenario(const wstring& filepath)
{
    ...
    if (filepath != L"MACRO")
    {
        ...
        if (fullpath == L"")
        {
            EM(ERROR_FILE_OPENFAIL_PATH_CANTFIND,filepath.c_str());
            return null;
        }
    }
    else
    ...
    IOData* data = Storages::GetFileText(fullpath);
//    kagchar* text = Storages::GetFileData(fullpath, &len);
    if (!data)
    {
        m_errorLine = -1;
        return null;
    }
    data->convertToWString();
    kagchar* text = data->getWBuffer();
    kagchar* ntext;
    ...
    {
        m_encoding = ENCODING_UTF16_LITE;
        ntext = text+2;
    }
    else if (text[0] == 0xff && text[1] == 0xfe)
    {
        m_encoding = ENCODING_UTF16_BIG;
        ntext = text+2;
    }
    else if (text[0] == 0xff && text[1] == 0xfe && text[2] == 0x0 && text[3] == 0x0)
    {
        m_encoding = ENCODING_UTF32_LITE;
        ntext = text+4;
    }
    else if (text[0] == 0x0 && text[1] == 0x0 && text[2] == 0xfe && text[3] == 0xff )
    {
        m_encoding = ENCODING_UTF32_BIG;
        ntext = text+4;
    }
    else
    {
    ...
    while (ntext)
    {
        ...
        if (m_errorLine != 0)
        {
            SAFF_DELETE_NULL(m_storage);
            m_label = null;
            m_tag = null;
            THROW_EM(m_errorInfo,m_errorLine);
            break;
        }
    }
    SAFF_DELETE_NULL(data);
    auto ret = m_storage;
    m_storage = null;
    m_label = null;
    m_tag = null;
...
void KAGParser::returnCall()
{
    ...
    {
        THROW_EM(ERROR_KAG_LABEL_FIND_FAIL,m_currentLine);
    }
